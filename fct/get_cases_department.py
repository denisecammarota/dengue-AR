# Code to separate cases of dengue or zika by municipality
# that is, output is epidemic curve in a municipality as a whole 
...
def get_cases_country(disease, id_department):
    # disease is either dengue (0) or zika (1) numeric
    # id_department is the department ID according to INDEC string
    ...
    df_cases = pd.read_csv(file_cases, dtype={'departamento_id': str})
    ...
    filt_cases = (df_cases['departamento_id'] == id_department)
    df_cases = df_cases[filt_cases] 
    df_cases = df_cases.drop(columns = ['departamento_id'])
    ...
    # adding department id into the results
    df_final['ID_DEPARTMENT'] = id_department
